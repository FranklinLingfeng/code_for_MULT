...
def process_sysu_multi_shot(data_path, mode='query', relabel=False):

    test_ids = open(os.path.join(data_path, 'exp', 'test_id.txt')).readline()
    selected_ids = test_ids.strip('\n').split(',')

    selected_ids = [int(i) for i in selected_ids]
    num_ids = len(selected_ids)

    img_paths = glob(os.path.join(data_path, '**/*.jpg'), recursive=True)
