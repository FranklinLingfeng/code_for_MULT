...
class BaseResNet(nn.Module):
    ...
    def forward(self, x_rgb, x_ir, mode=None, in_train=False):
        if mode == None:
            ...
            return feat_rgb, feat_ir, dis_rgb, dis_ir
        
        elif mode == 'RGB':
            x_rgb = self.visible_module(x_rgb)
            pool_rgb_eval, feat_rgb_eval = self.forward_main_net(x_rgb, in_train)
            return pool_rgb_eval, feat_rgb_eval
        
        elif mode == 'IR':
            x_ir = self.thermal_module(x_ir)
            pool_ir_eval, feat_ir_eval = self.forward_main_net(x_ir, in_train)
            return pool_ir_eval, feat_ir_eval

    def forward_main_net(self, x, in_train=False):
        ...
        # x_pool_4 = x_pool.view(x_pool.size(0), x_pool.size(1))
        ...
        if self.training:
            # feat_out = torch.cat((feat_0, feat_1, feat_2, feat_3, feat_4), dim=1)
            adnet_out = self.adnet(feat, train_set=True)
            x_dis = self.disnet(adnet_out)
            return feat, x_dis
        else:
            return x_pool, feat_eval